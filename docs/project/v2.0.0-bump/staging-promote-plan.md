...
| **Status** | ✅ **§4 branch promote and §5 staging deploy both complete, §6 UAT passed** (2026-09-05); release-prep (CHANGELOG consolidation + version bumps, PR #131) merged to `staging` and **R2 (`staging` → `main`) done** 2026-09-06 — see [`r2-promote-main/index.md`](r2-promote-main/index.md). `staging` was force-updated to `proj/v2.0.0-bump`'s tip via **M1** (Q5 resolved by action). The operator cutover (backups, user migration, `barrins_api`/`tamiyo_scroll`/`goblin_guide`/`barrins_identity` deploys) ran end to end on staging — see §5.2 for two real issues hit and fixed along the way. Full UAT checklist (§6) passed, including team rosters, S8–S18 smoke, and the Goblin Guide back-link. Q3 (main-promotion timing) resolved — see §8. |
...
| ~~Q3~~ | ~~Is `main` promotion (`staging` → `main`) in the same window, or a later cut?~~ **Resolved: same window** — user decision 2026-09-05 ("Same window, continue to production now"), after staging UAT (§6) passed cleanly with no regressions found. `main` was promoted 2026-09-06 (R2, [`r2-promote-main/index.md`](r2-promote-main/index.md)); the production `barrins_api` `users`-table drop itself happens later, at R4 (deploy from tag), not at this branch-promote step. | user |
