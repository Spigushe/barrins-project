...
| T8 | Deployment playbooks for Barrin's Scripture (scheduled job, not a web service) and Karn Tablets (real ML service per I4, shape TBD by T6) | T1 (done), T6, D1 (done) | 🟢 **Both playbooks written; neither deployed to the real VPS yet.** Barrin's Scripture half done 2026-08-08 (see below). **Karn Tablets half done 2026-08-28** — `ops/my-server/karn_tablets.yml` + `roles/karn_tablets/` run `apps/karn_tablets` as a daily `systemd`-timer job (03:00 UTC, `deploy_env` staging/production), same shape as `scripture_scraper` minus the scraping/Chromium/archive/sweep. `KARN_INGEST_TOKEN` via a new `karn_ingest_token` role (mirrors `scripture_ingest_token`); `KARN_TABLETS_DATABASE_URL_RO` is a hand-created read-only Postgres role. `karn` CI job added. `ansible-lint` clean. The three public read routes it feeds are already covered by `barrins_api.yml`'s `/bff/tolaria-news` rate-limit prefix — nothing added. **ADR-15's Jupyter Lab half is out of T8 scope**, split to T9. — Barrin's Scripture detail: `scripture_scraper` (shipped during T1) walked against D1's checklist (2026-08-05); the T3 sweep now runs on its own timer (independent of the daily scrape). `SCRIPTURE_INGEST_TOKEN` is now shared via the new `scripture_ingest_token` role (one value per environment, `secrets/scripture/`) rather than duplicated per app — supersedes this page's original per-app-file decision, same day. **ADR-12 (2026-08-10)**: after mtgo.com started blocking the VPS's static outbound IP, scrape+sweep scheduling moved off the VPS's systemd timers entirely, onto `.github/workflows/scripture-scrape.yml` (GitHub Actions' rotating runner IPs are unaffected) — the VPS's `scripture_scraper` role stays in the repo, dormant, as a rollback path only. Remaining: failure-notification (deliberately deferred to D2/F1, 2026-08-07 decision) — partially covered already, since a scheduled GitHub Actions workflow's failure emails the repo owner by default | [t8-scripture-karn-playbooks/](t8-scripture-karn-playbooks/index.md) |
| T9 | Karn Tablets Jupyter Lab workbench at `karn-jupyter.barrins-codex.org` — an interactive exploration tool over `bs_*`/`kt_*`, `admin`/`ml_developer` only, **not** part of the scheduled job | T6 (done), T8 (done), `auth_roles.md` | 🔲 **Not started** — split out of T8 (2026-08-28). This is the "T8-style implementation task" [ADR-15](../../content/ops/architecture/decisions.md#adr-15-karn-tablets-observability--job-health-and-jupyter-lab) defers the Jupyter deployment (and its auth-enforcement mechanism) to; it had no tracking page until now. Needs a new `ops/my-server` role + likely a `karn_jupyter.yml` playbook, `register_ssl` + nginx vhost + DNS for the subdomain, and a Jupyter access secret. The run-health half of ADR-15 is already closed (folded into D2/F1) | [t9-karn-jupyter-workbench/](t9-karn-jupyter-workbench/index.md) |
| T10 | Barrin's Identity — **service** implementation ([ADR-16](../../content/ops/architecture/decisions.md#adr-16-adopt-barrins-identity-as-the-rs256-jwks-authority), [ADR-17](../../content/ops/architecture/decisions.md#adr-17-shared-code-lives-in-a-top-level-libs-directory)); docs landed ahead of the code | ADR-16, ADR-17; blocks T9 | 🟩 **Done (2026-08-29)**. `apps/barrins_identity/` + shared `libs/identity_client/` copied over from `feat/barrins-identity` + `claude/barrins-identity-lifecycle-settings-4g2lyh` (path checkout, not cherry-pick), reconciled with current conventions, `username` handle added (migration `f6a7b8c9d0e1`, `Q-03`). 313 identity tests / 98.72%, 26 `identity_client` tests / 100%; `ruff`/`ty`/`bandit` clean; new `identity` CI job. **Out of scope, still deferred:** the `barrins_api` cutover ([platform.md §10](../../content/back/barrins_identity/platform.md#10-cutover)), the `ops/my-server/barrins_identity.yml` playbook + Brevo/OVH email, and the Goblin Guide frontend | [t10-barrins-identity/](t10-barrins-identity/index.md) |
| T11 | Goblin Guide — **frontend** (login + account management for Barrin's Identity); [ADR-17](../../content/ops/architecture/decisions.md#adr-17-shared-code-lives-in-a-top-level-libs-directory), [bootstrap.md](../../content/front/goblin_guide/bootstrap.md) | T10 (done); one of T10's three deferred phases | 🟨 **Login slice done (2026-08-29)** — `libs/goblin_guide/` (`@barrins/goblin-guide`, Vite library mode, 17 tests) + `apps/goblin_guide/` (standalone shell, `file:` path dep, 3 tests). `G-03` step 1: `LoginScreen` + `IdentityProvider` + `useLogin`/`useCurrentUser`/`useLogout`, single-flight silent refresh, pluggable `TokenStore` (in-memory default, `G-05`), token-driven `styles.css`. New `goblin_guide` CI job (library then shell) in `ci-required`. `G-03` delivery order locked. **Live UAT (rollout Phase 6):** login / refresh / on-load session restore validated on `goblin-staging` + `identity-staging` 2026-09-01. **Not done (this item):** the later `G-03` slices continue in T12; not yet mounted in `tamiyo_scroll` / `tolaria_news`; no deploy playbook | [t11-goblin-guide/](t11-goblin-guide/index.md) |
| T12 | Goblin Guide — **signup + email verification** (`G-03` step 2); [bootstrap.md](../../content/front/goblin_guide/bootstrap.md), [integration.md §4.2](../../content/back/barrins_identity/integration.md#42-self-registration-and-email-verification) | T11 (done) | 🟨 **Signup slice done (2026-08-30)** — `SignupScreen` + `VerifyEmailScreen` and `useSignup`/`useVerifyEmail`/`useResendVerification` over `POST /api/v1/auth/signup`, `/signup/verify`, `/signup/resend`. `username` field + a client-side password-rule checklist (UX feedback only); verify screen mirrors the 60s resend cooldown and reads the `?email=`/`?code=` deep link. Shell adds `/signup` + `/verify-email` routes. Two adjacent client bugs fixed: the `{ error: { message } }` error envelope, and the stale `placeholder` role. Library 39 tests / shell 6 tests, `oxlint` + `prettier` clean. **Live UAT (rollout Phase 6):** signup → real inbox → verify deep link, resend cooldown, wrong code validated on `goblin-staging` 2026-09-01. **Not done:** account settings + delete, admin service-account management (remaining `G-03` slices); host mounting; deploy playbook | [t12-goblin-guide-signup/](t12-goblin-guide-signup/index.md) |
| T13 | Goblin Guide — **password reset** (`G-03` step 3); [bootstrap.md](../../content/front/goblin_guide/bootstrap.md), [integration.md §4.3](../../content/back/barrins_identity/integration.md#43-password-reset) | T12 (done) | 🟨 **Password-reset slice done (2026-08-30)** — `ForgotPasswordScreen` + `ResetPasswordScreen` and `usePasswordResetRequest` / `usePasswordResetConfirm` over `POST /api/v1/auth/password-reset/request`, `/password-reset/confirm`. Forgot screen shows the service's generic confirmation verbatim (no account-existence disclosure); reset screen reads the `/reset-password?email=&code=` deep link (confirmed against `apps/barrins_identity` `_build_reset_link`), and a successful confirm returns a fresh pair (every other session revoked, `token_version` bump). The password-rule checklist (`PasswordRules`) and the digit-masked code field (`CodeField`) extracted into shared components on their third use; `SignupScreen` / `VerifyEmailScreen` refactored onto them. `KeyIcon` added. Shell adds `/forgot-password` + `/reset-password` routes; `LoginRoute` wires `onForgotPassword`. Library 57 tests / shell 9 tests, `oxlint` + `prettier` clean. **Live UAT (rollout Phase 6):** request → email → reset deep link → confirm → old token `401`, resend cooldown validated on `goblin-staging` 2026-09-01. **Not done:** admin service-account management (last `G-03` slice); host mounting; deploy playbook | [t13-goblin-guide-password-reset/](t13-goblin-guide-password-reset/index.md) |
| T14 | Goblin Guide — **account settings + delete** (`G-03` step 4); [bootstrap.md](../../content/front/goblin_guide/bootstrap.md), [integration.md §4.4](../../content/back/barrins_identity/integration.md#44-account-management) | T13 (done) | 🟨 **Account-settings slice done (2026-08-30)** — `AccountScreen` and `useUpdateAccount` / `useVerifyEmailChange` / `useResendEmailChange` / `useDeleteAccount` over `PATCH /api/v1/users/me`, `/users/me/email-change/verify` + `/resend`, `DELETE /api/v1/users/me`. Inline display-name edit (check-button commit); two-step email change with the pending address held apart from `user.email`, the shared `CodeField`, and a mirrored 60s resend cooldown; password-gated delete that clears the token store and routes to `/login?deleted=1` (new `LoginScreen` `accountDeleted` banner). Shell mounts `AccountScreen` at `/` in place of the read-only card and adds a `/confirm-email-change` deep-link route behind `RequireAuth`; `RequireAuth` now carries a `?next=` return path. One new token `--gg-danger` + `.gg-button-danger`. Authenticated password change is **out of scope** (no backend endpoint — use password reset). Library 73 tests / shell 11 tests, `oxlint` + `prettier` clean. **Live UAT (rollout Phase 6):** display-name change (empty clears → NULL), email-change verified at the new address, delete → `/login?deleted=1` validated on `goblin-staging` 2026-09-01. **Not done:** host mounting; deploy playbook | [t14-goblin-guide-account-settings/](t14-goblin-guide-account-settings/index.md) |
| T15 | Goblin Guide — **admin service-account management** (`G-03` step 5 / `G-04`); [bootstrap.md](../../content/front/goblin_guide/bootstrap.md), [integration.md §4.6](../../content/back/barrins_identity/integration.md#46-service-accounts) | T14 (done) | 🟨 **Service-account slice done (2026-08-30)** — `ServiceAccountsScreen` and `useServiceAccounts` / `useCreateServiceAccount` / `useRevokeServiceAccount` over `GET`/`POST /api/v1/service-accounts` + `/{client_id}/revoke`. A `useCurrentUser()` gate: `admin` gets the list (revoked accounts kept, badged, no Revoke action), a create form (optional description + an at-least-one scope tag input), a one-time `client_secret` panel with copy buttons, and a danger-styled revoke confirm; everyone else gets an "administrator access required" panel. Frontend only — backend already on the release line (`apps/barrins_identity/app/api/v1/service_accounts.py`). Shell extracts `ShellFrame` (shared header, admin-only Account/Service-accounts nav toggle) and mounts the screen at `/service-accounts` behind a `?next=`-aware `RequireAuth`. New tag-input / credential / list-status CSS, no new token; `CopyIcon` + `CloseIcon` added. `POST /service-token` deliberately not surfaced (machine-to-machine only). Library 85 tests / shell 14 tests, `oxlint` + `prettier` clean. Completes `G-03`. **Live UAT (rollout Phase 6):** admin create → one-time secret → list → revoke, non-admin access panel validated on `goblin-staging` 2026-09-01. **Not done:** host mounting in `tamiyo_scroll` / `tolaria_news`; deploy playbook | [t15-goblin-guide-service-accounts/](t15-goblin-guide-service-accounts/index.md) |
| T16 | Goblin Guide — **feature gate engine** (admin-toggleable feature flags, runtime on/off layered on top of the existing static `Role` floor); scoped alongside [#123/#124](../../issue-scoping/123-124-structured-match-log.md) | T15 (admin-screen pattern) | 🔲 **Scoped — decision needed (2026-09-15).** Not started. Registry recommended in `barrins_identity` (`feature_gates` table + `GET`/`PATCH /api/v1/feature-gates`), consumed via `libs/identity_client`, admin screen in Goblin Guide mirroring T15's `ServiceAccountsScreen`. Deliberately does **not** block #123/#124, which ships gated at a static `moderator` floor now (existing `require_role()`/`ModeratorUser` infra) and becomes this engine's first registered gate once T16 lands | [t16-goblin-guide-feature-gates/](t16-goblin-guide-feature-gates/index.md) |
...
| S19 | Fold/unfold the Decklist tab's "Current decklist" section — a chevron toggle in the section header collapses the card tables, keeping the header (version badge, legend, PDF button, "card change being considered" block) as the unfold anchor; state remembered per browser (`localStorage`, not account-synced), like S12's display prefs — added 2026-09-08 from a chat request, not a GitHub issue | S4 (`CurrentDecklistSection`/`DecklistViewContent`) | 🔲 **Phase 1 in progress** (add the control, default expanded). Phase 2 — flip the default to collapsed so unfolding is the deliberate action — deferred to a follow-up commit. `apps/tamiyo_scroll`-only, no schema/API/dependency change | [s19-decklist-fold/](s19-decklist-fold/index.md) |
...
| R1 | Finalize release content, merge `proj/v2.0.0-bump` → `staging` | All of Groups T/S/F/D above that are in scope, **and R5** (ADRs merged before this point, not after — §3.1) | ✅ Done, 2026-09-05/06 — [r1-merge-staging/](r1-merge-staging/index.md) |
| R2 | Promote `staging` → `main` | R1 | ✅ Done, 2026-09-06 — [r2-promote-main/](r2-promote-main/index.md) |
| R3 | Tag and cut the release | R2 | 🟡 Tag done, release drafted (publish deferred to R4), 2026-09-06 — [r3-tag-release/](r3-tag-release/index.md) |
