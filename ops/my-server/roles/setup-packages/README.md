...
any app-specific playbook — every app role (`register-ssl`,
`fastapi-backend`, `react-frontend`, ...) assumes `nginx`/`certbot`/build
...
- A Debian host reachable over SSH as a user with sudo (normally
  `username` from `setup-base-user`, run right after it in `setup.yml`).
