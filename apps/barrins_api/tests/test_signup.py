...
Covers 100% of the POST /auth/signup, /auth/signup/verify, and
/auth/signup/resend routes (app/api/v1/routers/auth.py), as well as the
app/models/email_verification.py model.
"""
...
class TestSignupWithVerificationDisabled:
    ...
    async def test_creates_already_verified_account_and_returns_tokens(
        ...
        # No email sent, no EmailVerification row created.
        assert fake_email_sender.sent == []
...
class TestVerifySignup:
    ...
    async def test_missing_verification_record_returns_400(
        ...
        user = User(
            ...
            hashed_password="irrelevant",  # noqa: S106 — not a password, not checked in this test
            is_verified=False,
        )
        db_session.add(user)
        await db_session.commit()
...
class TestResendVerification:
    async def test_generates_new_code(
        ...
        resp = await client.post(
            ...
            json={"email": "newplayer@example.com"},
        )
    ...
    async def test_cooldown_active_returns_generic_202_without_sending(
        ...
        assert resp.status_code == 202
        assert len(fake_email_sender.sent) == 1  # no second send (active cooldown)
