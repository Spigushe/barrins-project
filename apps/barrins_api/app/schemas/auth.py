...
# Single source of truth — shareable with the frontend via GET /openapi.json.
# Note: Pydantic v2 uses the Rust engine which doesn't support look-arounds;
# Field(pattern=...) is reserved for OpenAPI exposure, actual validation
# goes through AfterValidator.
...
def _check_password_complexity(v: str) -> str:
    """Complexity validator — used via Annotated to avoid duplication."""
    if not PASSWORD_PATTERN.fullmatch(v):
        raise ValueError(PASSWORD_RULE)
    return v


# Reusable type (Pydantic v2 idiom) — shared by UserCreate and UserSignup.
# No min_length=12: the regex .{12,}$ covers the constraint and unifies the
# error messages (a single PASSWORD_RULE, whatever the failure cause).
PasswordStr = Annotated[
    str,
    Field(
        json_schema_extra={
            "pattern": PASSWORD_PATTERN.pattern,  # OpenAPI only
...
class UserCreate(BaseModel):
    ...
    Exposes the privileged fields (role, is_verified) because the caller
    is an admin. Never expose this schema on a public endpoint.
...
class UserSignup(BaseModel):
    """Self-registration payload from the frontend.
    ...
    ⏳ Future endpoint — the route isn't registered in the router.
    """
...
class TokenData(BaseModel):
    """Decoded content of the JWT payload (internal claims)."""
...
class SignupResponse(BaseModel):
    ...
    present — the client is logged in immediately, no additional call
    needed.
    """
...
class ResendVerificationResponse(BaseModel):
    ...
    The message never varies based on whether the account exists (constitution §23.2).
    """
